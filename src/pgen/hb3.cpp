    ...
    for (int i=is; i<=ie; i++) {
      ...
      if (NDUSTFLUIDS > 0) {
        for (int n=0; n<NDUSTFLUIDS; ++n) {
          ...
          pdustfluids->df_cons(rho_id, ks, j, i)  = rd;
          pdustfluids->df_cons(v1_id,  ks, j, i)  = rd*rvx;
          pdustfluids->df_cons(v2_id,  ks, j, i)  = rd*rvy;
          pdustfluids->df_cons(v3_id,  ks, j, i)  = rd*rvz;
          pdustfluids->df_cons(v3_id,  ks, j, i) -= rd*qshear*Omega_0*x1;
        }
      }
