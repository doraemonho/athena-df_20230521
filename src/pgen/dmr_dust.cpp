      ...
      if (pcoord->x1f(i) > shock_xpos_top) {
        ...
        if (NDUSTFLUIDS > 0) {
          for (int n = 0; n<NDUSTFLUIDS; n++) {
            ...
            pdustfluids->df_cons(rho_id, ks, j, i) = initial_D2G[n]*phydro->u(IDN,ks,j,i);
            pdustfluids->df_cons(v1_id,  ks, j, i) = initial_D2G[n]*phydro->u(IM1,ks,j,i);
            pdustfluids->df_cons(v2_id,  ks, j, i) = initial_D2G[n]*phydro->u(IM2,ks,j,i);
          }
        }
