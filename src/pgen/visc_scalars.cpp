...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  x0 = pin->GetOrAddReal("problem","x0",1.0);
  t0 = pin->GetOrAddReal("problem","t0",0.5);
  A0 = pin->GetOrAddReal("problem","A0",0.5);
  sig = pin->GetOrAddReal("problem","sigma_b",0.2);
      ...
      for (int j=js; j<=je; ++j) {
        for (int i=is; i<=ie; ++i) {
          if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
            ...
            //phydro->u(IDN,k,j,i) = 1.2*A0*std::exp(-SQR(x1-x0)/(2.0*SQR(sig)));
            ...
            if (NSCALARS > 0) {
              Real concentration = A0*std::exp(-SQR(x1-x0)/(2.0*SQR(sig)));
              //Real concentration = 0.9;
