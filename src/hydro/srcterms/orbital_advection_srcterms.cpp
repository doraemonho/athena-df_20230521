...
#pragma omp simd
            for (int i=pmb->is; i<=pmb->ie; ++i) {
              ...
              //// TODO I changed the orbital advection in 3D cylindrical
              //const Real &den   = prim(IDN, k, j, i);
              //const Real &rv    = pmb->pcoord->x1v(i);
              ...
              //const Real mom1   = den*prim(IVX, k, j, i);
              //const Real &vp    = prim(IVY, k, j, i);
              //const Real flux_c = 0.5*(flux[X1DIR](IDN, k, j, i+1)+flux[X1DIR](IDN, k, j, i));
              //cons(IM1, k, j, i) += 2.0*dt*vc*ri*(den*vp);
              //cons(IM2, k, j, i) -= dt*(vc*ri+0.5*rdok_i)*(mom1+flux_c);
              //if (NON_BAROTROPIC_EOS) {
