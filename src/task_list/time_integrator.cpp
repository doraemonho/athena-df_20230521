...
TimeIntegratorTaskList::TimeIntegratorTaskList(ParameterInput *pin, Mesh *pm) {
  ...
  {using namespace HydroIntegratorTaskNames; // NOLINT (build/namespace)
    ...
    if ((NDUSTFLUIDS > 0) && (NSCALARS > 0)) {
      ...
      AddTask(SRCTERM_HYD,(INT_HYD|INT_DFS|INT_SCLR));
      ...
      AddTask(DRAG_DUSTGAS,(SRCTERM_DFS|SRCTERM_HYD)); // The drag term is after the srcterms
    } else if (NSCALARS > 0) {
    ...
    if (MAGNETIC_FIELDS_ENABLED) { // MHD
        ...
        if (SHEAR_PERIODIC) {
          if ((NDUSTFLUIDS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_FLD|RECV_FLDSH
                            |SEND_DFS|RECV_DFSSH|SEND_SCLR|RECV_SCLRSH));
          } else if (NDUSTFLUIDS > 0) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_FLD|RECV_FLDSH
                            |SEND_DFS|RECV_DFSSH));
          } else if (NSCALARS > 0) {
          ...
          if ((NDUSTFLUIDS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_FLD|SETB_FLD
                            |SEND_DFS|SETB_DFS|SEND_SCLR|SETB_SCLR));
          } else if (NDUSTFLUIDS > 0) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_FLD|SETB_FLD
                            |SEND_DFS|SETB_DFS));
          } else if (NSCALARS > 0) {
      ...
      if (pm->multilevel) { // SMR or AMR
        if (SHEAR_PERIODIC) {
          if ((NDUSTFLUIDS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|RECV_HYDSH|SEND_DFS|RECV_DFSSH|SEND_SCLR|RECV_SCLRSH));
          } else if (NDUSTFLUIDS > 0) {
          ...
          if ((NDUSTFLUIDS > 0) && (NSCALARS > 0)) {
            AddTask(PROLONG,(SEND_HYD|SETB_HYD|SEND_DFS|SETB_DFS|SETB_SCLR));
          } else if (NDUSTFLUIDS > 0) {
        ...
        if (SHEAR_PERIODIC) {
          if ((NDUSTFLUIDS > 0) && (NSCALARS > 0)) {
            AddTask(CONS2PRIM,(RECV_HYDSH|RECV_DFSSH|RECV_SCLRSH));
          } else if (NDUSTFLUIDS > 0) {
...
TaskStatus TimeIntegratorTaskList::AddSourceTermsHydro(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if (stage_wghts[stage-1].main_stage) {
      ...
      ph->hsrc.AddHydroSourceTerms(t_start_stage, dt, ph->flux, ph->w, pdf->df_prim, ps->r,
                                  pf->bcc, ph->u, pdf->df_cons, ps->s);
      pmb->DeepCopy(ph->u_af_src, ph->u);
    }
    return TaskStatus::next;
  }
...
TaskStatus TimeIntegratorTaskList::PhysicalBoundary(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    // Scaled coefficient for RHS time-advance within stage
    Real dt = (stage_wghts[(stage-1)].beta)*(pmb->pmy_mesh->dt);
    // Swap Hydro and (possibly) DustFluids, passive scalar quantities in BoundaryVariable interface
    // from conserved to primitive formulations:
